...
pass  # This prevents VScodes import optimizer from mixing the following imports with those above
from src.parameter_sets.par import *
from src.tools import crosscorr
...
model_path_id.mkdir(exist_ok=True)
...
train_cgm(config, max_epochs= 30, grace_period=5, n_epochs_stop=15, data_obj=data_obj, useRayTune=False)

# Load best model
model_state, optimizer_state = torch.load(os.path.join('./src/model_state_tmp', "checkpoint"))
model.load_state_dict(model_state)

# Copy the trained model to model path
copyfile(os.path.join('./src/model_state_tmp', "checkpoint"), model_path_id / 'checkpoint')

with open(os.path.join('./src/model_state_tmp', "hyperPars.json"), 'w') as fp:
    json.dump(config, fp)
